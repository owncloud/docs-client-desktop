= Configuration File
:toc: right
:description: The Desktop App uses a configuration file. It has several sections for particular settings.
...
{description} You will find more sections in the configuration file than described here. Do not change any of those settings except support advises you to do so. 
...
| *Microsoft Windows* 
a| `%APPDATA%+\+{ini_name}+\+{bin_name}.cfg`
| *macOS* 
a|`$HOME/Library/Preferences/{ini_name}/{bin_name}.cfg`
| *Linux* 
a| `$HOME/.config/{ini_name}/{bin_name}.cfg`
|===

The configuration file contains settings using the {ini-file-format-url}[Microsoft Windows .ini file format]. Some settings can be defined using the {ini_name} Desktop App configuration dialog.

...
:toc: right
:description: When using the Desktop App, depending on the operating system (OS) you are using, file and folder names respectively their lengths can have different restrictions.

:control_code_chart-url: https://en.wikipedia.org/wiki/ASCII#Control_code_chart

== Introduction

{description} Creating files and folders with allowed names or max lengths on one OS, may have issues or even can´t be synced because of different rules in another OS. This page gives you a brief overview of limitations of different OS for file and folder names.

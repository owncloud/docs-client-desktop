...
When removing a synchronisation connection, the connection will get removed but all data synchronized will stay locally and does not get deleted. To remove a sync relationship, change to the account and select the synchornisation relationship that should get removed. When clicking the btn:[...] (three dots) icon, the following window appears:
...
On Windows, you may get asked for elevated permissions deleting that folder. Follow the procedure shown or ask you administrator for assistance. In rare cases, the folder denies deletion even elevated permissions were given. In such a case, mark that folder in Explorer and press btn:[Shift + Delete] together and follow the procedure shown. This will delete that folder without using the trashbin.

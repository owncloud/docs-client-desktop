...
When removing a synchronisation connection, the connection will get removed but all data synchronized will stay locally and does not get deleted. To remove a sync relationship, change to the account and select the synchornisation relationship that should get removed. Clicking the btn:[...] (three dots) icon, the following window appears:

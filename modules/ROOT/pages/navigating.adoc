...
Automatic Babdwith limiting: When activated, the client limits the upload- or download bandwidth to 25% of the currently available bandwidth of each operation. The available bandwidth is measured on the fly at the beginning of every operation for a short time period.

...
If you have removed the Desktop App but still have the {ini_name} shortcut or symbol in the Windows Navigation Side Bar, here is how you to remove it:

1. Open `regedit`. (press the Windows Key and type regedit)
2. Search (CTL+F) for the name of the Desktop App, in this case `{bin_name}`.

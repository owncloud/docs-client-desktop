...
It may happen that the Desktop App unexpectedly crashes due to unforseen or undhandled cirumstances. In such a case, a crash report is generated. This report contains valuable information for ownCloud to debug the root cause. This report is not sent to ownCloud automatically by the Desktop App, the user has to confirm to do so. When the user has agreed to send the crash report, he gets a reference ID that can be used in communication with ownCloud for this issue.

Crash reports are available for following environments:
...
The following table shows how a crash report window looks like and how when it has been sent.

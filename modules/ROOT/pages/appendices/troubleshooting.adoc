...
NOTE: That the choice to enable logging will be persisted across client restarts.
...
If the ownCloud client isn't able to start and immediately crashes the first two options are not available. Therefore, it might be necessary to start the ownCloud client using the command line in order to see the error message

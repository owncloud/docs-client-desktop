...
You can also use the {squish-docker-image-url}[squish docker image] to run tests. Just proceed with following steps:

Pull the docker image with following command:
...
Some necessary steps before running test.
...
. Click btn:[Add] and select the compiled binary, eg: `client/client-build/bin/owncloud`
. Close any running Desktop Client, by clicking btn:[Quit ownCloud] in the settings page

...
* As a user, go to the Personal Settings page and download if provided the appropriate build if it has not already been installed by your company.
...
Note that the filenames shown in the following sections are examples only and may need to be replaced with their real names.

=== Installation on Windows

* When deploying centrally, skip this section and see xref:deploying-on-windows[Deploying on Windows] for configuration options of the installer.
...
Compare {ms-remove-url}[REMOVE, window=_blank] and {ms-adddefault-url}[ADDDEFAULT, window=_blank] at the Windows Installer Guide.

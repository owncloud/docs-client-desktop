...
:toc: right
:toclevels: 3
:description: This documentation will guide you through the deployment or installation of the Desktop App.
...
a| *Linux* +
(x86-64)
a| * The **xref:appimage[AppImage]** build of the Desktop App is available to support most Linux platforms
|===
...
You can download the latest version of the Desktop App from the {desktop-clients-url}[Desktop App Download] page. There are Desktop Apps for _Linux_, _macOS_, and _Microsoft Windows_ available. When using Linux, you can also use AppImage.
endif::[]

ifeval::["{targetbuild}" == "kw"]
To get the latest version of the Desktop App... +
[.yellow]#@oksana, we need to define and describe how / where the source at KW can be downloaded#
endif::[]

=== Configuration Files

Regardless of the type of physical installation, a configuration file will be created when the app is first started. The location of the configuration file is described in the xref:advanced_usage/configuration_file.adoc[Configuration File] documentation.

* Deleting an existing configuration file will not delete any locally synchronized data, but it will launch the configuration wizard which is described in the User Guide and you will need to re-configure.
...
Installation on macOS and Windows is the same as for any software application: download the installer,  double-click it to launch the installation and follow the installation wizard

ifeval::["{targetbuild}" == "oc"]
For Windows, once installed and configured, the Desktop App will update itself automatically unless you manually disable it. For more information about this setting see the xref:disabling-automatic-updates[Disabling Automatic Updates] section.
endif::[]

For Windows when using a centralized deployment, see the xref:customize-the-windows-deployment-or-installation[Customize the Windows Deployment or Installation] section, which describes the configuration options of the installer.
...
** For distributions such as Ubuntu +22.04 or Debian +11, use the command below to install the extension:
+
[source,bash]
----
...
** For distributions based on Red Hat Enterprise Linux (including AlmaLinux and Rocky Linux), you must first install a different package:
+
--
[source,bash]
----
...
** Once the extension is installed, you will need to log out and log in again to activate the changes.
For this particular system tray icon extension, the name of the extension displayed may vary and may be such as **Ubuntu AppIndicators** or **AppIndicator and KStatusNotifierItem Support**. Once enabled, the system tray will be displayed again within the desktop environment.
+
...
Linux users will need to follow the instructions on the {desktop-clients-url}[download] page to add the appropriate repository for their Linux distribution, install the signing key, and use their package manager to install the Desktop App. Linux users will also update their Desktop App using their package manager. The Desktop App will display a notification when an update is available. See xref:installing-shell-integration-packages[Installing Shell Integration Packages]. Overlay icons and a special shortcut menu for your file browsers must be installed manually.

You can also find links to source code archives and older versions on the download page.

On Debian-based systems, it may happen after a while that running `sudo apt update` returns a message about a signature verification error. This can be solved by updating the keys. On Debian, use the following command to update all signatures:
...
ifeval::["{targetbuild}" == "oc"]
You can download the AppImage at the {desktop-clients-url}[Linux section of the Download Desktop App] page.
endif::[]
ifeval::["{targetbuild}" == "kw"]
To get the latest version of the Desktop App... +
[.yellow]#@oksana, we need to define and describe how / where the AppImage source at KW can be downloaded#
endif::[]
...
AppImages require one package to be installed before usage which is `libfuse2t64`. Check the following procedure if it is already present and how to install if not.
...
ifeval::["{targetbuild}" == "kw"]
* Download the latest version of the Desktop App and move it to the `Applications` directory: +
[.yellow]#@oksana, we need to define and describe how / where the AppImage source at KW can be downloaded#
endif::[]
...
ifeval::["{targetbuild}" == "oc"]
* Automatically update the AppImage: +
For automatic updating of the AppImage, see the xref:automatic_updater.adoc#linux[Automatic Updating of the Desktop Application - Linux] documentation.
endif::[]
...
== Customize the Windows Deployment or Installation

If you just want to install the Desktop App on your local system, you can simply {client-download-url}{url-version}/win/[download,window=_blank] and launch the relevant `.msi` file, run and configure it in the wizard that pops up. The examples below use the x64 installer. +
[.yellow]#@oksana, we need to define and describe how / where the source at KW can be downloaded#

=== Microsoft GPO (Group Policy Object) Installer

{ini_name} provides two versions of the Microsoft Installer:

- `*.x64.msi`: +
This is the installer for regular users. The installer UI has multiple translations to guide end users through the install procedure.
- `*.x64.GPO.msi`: +
...
The MSI installer provides several features that can be installed or removed individually, which you can also control from the command line if you want to automate the installation, then run the following command:
...
This command installs the Desktop App in the default location with the default features enabled. For example, if you want to disable desktop shortcut icons, you can simply change the above command to the following:
...
[source,console,subs="attributes+"]
----
msiexec /passive /i {bin_name}-{client-version}.x64.msi NO_DESKTOP_SHORTCUT="1"
...
Windows keeps track of the installed features and using `REMOVE` or `ADDDEFAULT` will only affect the features mentioned.

Compare {ms-remove-url}[REMOVE, window=_blank] and {ms-adddefault-url}[ADDDEFAULT, window=_blank] on the Windows Installer Guide.

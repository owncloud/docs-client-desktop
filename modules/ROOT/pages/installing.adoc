...
:toc: right
:toclevels: 4
:client-version-win: 5.2.0.12726.x64
:client-version-appimage: 5.2.0.12726-x86_64
...
=== Microsft GPO (Group Policy Object) Installer

ownCloud provides two versions of the Microsoft Installer:

- `*.x64.msi`: This is the installer for regular users. Installer UI has multiple translations to guide end users through the install procedure.
- `*.x64.GPO.msi`: The GPO installer works better for automated deployments in managed environments. It only has English language. The desktop client itself comes with all the usual langauges in the user interface.

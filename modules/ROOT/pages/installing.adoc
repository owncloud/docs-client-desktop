...
=== Define an Own Synchronisation Folder Icon

When setting up a new synchronisation, ownCloud automatically assigns its icon to the synchronisation folder for ease of identification. Though you can change this icon, it would be reverted back on next reboot to the ownCloud icon. Folder icon details are usually stored in the hidden desktop.ini file which is located _inside_ the folder under scrutiny. To make a manually defined icon persistent, a small change in this `desktop.ini` file is necessary. See the following description to do so:
...
* Finally apply any icon of your choice to the synchronisation folder.

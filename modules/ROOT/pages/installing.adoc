...
inlude::partial$conditional-naming.adoc[]
...
* Deleting an existing configuration file will not delete any locally synchronized data, but it will launch the xref:installation-wizard[Installation Wizard] and you will need to re-configure.
...
* If you want to re-deploy from scratch, you can safely delete the configuration file if it exists. In this case, check for existing sync folders that may also need to be deleted.

=== Installation on macOS and Windows

Installation on macOS and Windows is the same as for any software application: download the installer,  double-click it to launch the installation and follow the installation wizard. Once installed and configured, the Desktop App will automatically update itself unless you manually disable it, which you can do on Windows. For more information about this setting see the xref:disabling-automatic-updates[Disabling Automatic Updates] section.
...
* You can add command line parameters when starting up the Desktop App post installation. One parameter to mention is the option `-s`. This option forces the settings page to be shown on startup. While not necessary during general usage, it can be helpful if system tray icons are not available any more within your desktop environment.
...
----
msiexec /passive /i {bin_name}-{client-version-win}.x64.msi
----
...
[source,console,subs="attributes+"]
----
msiexec /passive /i ownCloud-{client-version-win}.x64.msi REMOVE=DesktopShortcut
----
...
[source,console,subs="attributes+"]
----
msiexec /passive /i ownCloud-{client-version-win}.x64.msi ADDDEFAULT=Client
----
...
[source,console,subs="attributes+"]
----
msiexec /passive /i ownCloud-{client-version-win}.x64.msi ADDDEFAULT=StartMenuShortcuts
----
...
[source,console,subs="attributes+"]
----
msiexec /passive /i ownCloud-{client-version-win}.x64.msi NO_DESKTOP_SHORTCUT="1"
...
[source,console,subs="attributes+"]
----
msiexec /passive /i ownCloud-{client-version-win}.x64.msi ADDDEFAULT="DesktopShortcut"
----
...
[source,console,subs="attributes+"]
----
msiexec /passive /i ownCloud-{client-version-win}.x64.msi INSTALLDIR="C:\Program Files\Non Standard ownCloud Client Folder"
----
...
[source,console,subs="attributes+"]
----
msiexec /passive /i ownCloud-{client-version-win}.x64.msi SKIPAUTOUPDATE="1"
----
...
[source,console,subs="attributes+"]
----
msiexec /i ownCloud-{client-version-win}.x64.msi LAUNCH="1"
----
...
[source,console,subs="attributes+"]
----
msiexec /i ownCloud-{client-version-win}.x64.msi REBOOT=ReallySuppress
----
...
[source,console,subs="attributes+"]
----
msiexec /i ownCloud-{client-version-win}.x64.msi DO_NOT_SCHEDULE_REBOOT="1"
----
...
----
[.ShellClassInfo]
IconResource=C:\\Program Files\\kiteworks\\kiteworks.exe
----
--

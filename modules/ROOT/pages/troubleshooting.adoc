...
Performing a general test::
  The first step in troubleshooting synchronization issues is to verify that you can log on to the {ini_name} web application. To verify connectivity to the {ini_name} server try logging in via your Web browser. If you are not prompted for your credentials, or if a red warning box appears on the page, the server setup requires review. Please contact your administration departement and verify that the server installation is working correctly.

Ensure the WebDAV API is working::
  If all Desktop Apps fail to connect to the {ini_name} instance, but access using the Web interface functions properly, the problem is often a misconfiguration of the WebDAV API. The Desktop App uses the built-in WebDAV access of the server content. Verify that you can log on to ownCloud's WebDAV server. To verify connectivity with the ownCloud WebDAV server, open a browser window and enter the address to the ownCloud WebDAV server. For example, if your ownCloud instance is installed at `\https://yourserver.com/owncloud`, your WebDAV server address is `\https://yourserver.com/owncloud/remote.php/webdav`. If you are prompted for your username and password but, after providing the correct credentials, authentication fails, please ensure that your authentication backend is configured properly.
...
|===
a| image::troubleshooting/folder_properties.png[Folder Properties,width=200]
a| image::troubleshooting/advanced_security_settings.png[Advanced Security Settings,width=350]
|===
...
* Synchronizing the same directory with ownCloud and other synchronization software such as Unison, rsync, Microsoft Windows Offline Folders, or other cloud services such as Dropbox or Microsoft OneDrive is not supported and should not be attempted. In the worst case, it is possible that synchronizing folders or files using ownCloud and other synchronization software or services can result in data loss.
...
+
image:troubleshooting/log_output_window.png[image,width=350]
+
...
press btn:[F12] or btn:[Ctrl-L] or btn:[Cmd+L] on your keyboard.
+
image:troubleshooting/log-files-to-keep.png[CLog files to keep,width=350]

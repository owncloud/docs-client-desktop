...
----
./{bin_name}.AppImage --logdir /tmp/{bin_name}_logs --logdebug
----
...
* On Linux, simply open the terminal and run, replace the path and file name accordingly:
+
[source,bash,subs="attributes+"]
----
...
If you experience problems, you can run the installer with logging enabled:
...
The macOS installer comes with its own logging console. Just run the \*.PKG as usual. At the first install screen in the menu bar, click menu:Window[Installer Log (Command + L)].
...
The command for example on a Linux system:

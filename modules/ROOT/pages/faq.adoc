...
|===
a| With Infinite Scale, quotas are only shown for project spaces, not for personal spaces or received shares. Each space has its own sync relationship, and quota values are shown as defined for each space.
...
Due to technical limitations, the Desktop App cannot detect local changes if the local sync folder is located on a mounted network volume. Only full local discovery, which by default runs every hour if not changed, will detect changes and trigger the upload.

...
The Desktop App does not provide a way to change the local sync folder directly. However, there is a way to do it without fully resyncing. Do this for each synchronization connection that you want to move to a new local destination:


. First, select the synchronization connection and remove it. For details to do so see xref:using.adoc#removing-a-synchronization-connection[Removing a Synchronization Connection].

...
=== The Desktop Client doesn't sync changes from my local sync folder when I use a network volume

Due to technical limitations, the desktop sync client can't detect local chenges, when the local sync folder is stored on a mounted network volume. Only the full local discovery (default=1h) will detect the change and trigger the upload. You might configure a smaller [`fullLocalDiscoveryInterval`](https://doc.owncloud.com/desktop/next/advanced_usage/configuration_file.html#section-owncloud) to sync local changes more often.

...
----
`+C:\Users\user_name\+{ini_name}`    -> `+C:\Users\user_name\sync\+{ini_name}`
`+C:\Users\user_name\otherAccount+` -> `+C:\Users\user_name\sync\otherAccount+`
----
====
...
+
--
Note that with modern authentication methods such as OpenID Connect (OIDC), you will be redirected to a browser for authentication. Images and authentication methods may vary depending on your corporate setup:
...
It is easy to add more sync relationships if required.
...
To manage a sync relationship, click on the btn:[Three Dots] button to open the menu as shown in the screenshot.
...
Enable virtual file support for this sync relationship, if supported by the operating system.
endif::[]
...
Note that menu:mouse[right click] will opens a menu for more actions.
endif::[]
...
image:using/oc-server-notifications.png[Server Notifications,width=100]
endif::[]
...
+
image:using/oc-windows-settings-menu.png[advanced approval, width=100]
...
ifeval::["{targetbuild}" == "oc"]
IMPORTANT: The bandwidth settings are deprecated. When the last manually defined setting is reset to default, the entire bandwidth section will no longer be visible. The bandwidth section will be removed in a future release.
...
* Limit to
+
image:using/settings_network.png[Network Settings,width=100]
...
NOTE: Changing the global exclude definition file can render the client unusable or cause undesirable behavi

Each line in the editor contains an ignore pattern string. When creating custom patterns, in addition to using normal characters to define an ignore pattern, you can use wildcard characters to match values. For example, you can use an asterisk (`*`) to match any number of characters, or a question mark (`?`) to match a single character.

Patterns that end with a slash (`/`) are applied only to directory components of the path being scanned.
...
Each pattern string in the list is preceded by a checkbox. If the checkbox contains a check mark, not only will the file or directory component matching the pattern be ignored, but any matching files will also be considered "ephemeral metadata" and removed by the Desktop App.

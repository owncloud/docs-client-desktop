...
After the Desktop App has been started, it can not only be fully closed with the Quit image:using/quit-symbol.png[Quit Symbol, width=18] symbol on the top right, but also be closed to the system tray to run continuously in the background.
...
* A click on the {ini_name} menu bar icon opens the application menu.
* To show the application window, select *Show {ini_name}* from the menu.

==== Linux Desktop

* A btn:[left-click] or btn:[right-click] on the {ini_name} tray icon opens the application menu.

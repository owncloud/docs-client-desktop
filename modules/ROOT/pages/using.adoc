...
If an account exists, you can at any time add another account for a different server with the btn:[Add Account] button.
...
. The user and his credentials.
. The base location on your local file system to store the data for a sync connection.
+
[NOTE]
====
If you plan to use multiple accounts, consider using a common base location in the file system for all accounts, which makes them easier to identify via a root folder. This means that you do not take the default suggested path, but customize it to your needs when enabling the btn:[Advanced configuration].

.Example
[source,plaintext,subs="attributes+,macros+"]
----
`+C:\Users\xyz\+{ini_name}`    -> `+C:\Users\xyz\sync\+{ini_name}`
`+C:\Users\xyz\otherAccount+` -> `+C:\Users\xyz\sync\otherAccount+`
----
====
...
+
--
Note that with modern authentication methods such as OpenID Connect (OIDC), you will be redirected to a browser for authentication. Images may vary depending on your corporate setup:

.Login with OIDC (Follow the web login process)
ifeval::["{targetbuild}" == "oc"]
image:using/oc-wizard-screen-url.png[Form Entering your login, width=100]
image:using/oc-wizard-screen-grant-access.png[Form Granting Access, width=80]
endif::[]
ifeval::["{targetbuild}" == "kw"]
image:using/kw-wizard-screen-url.png[Form Entering your login, width=100]
...
ifeval::["{targetbuild}" == "oc"]
Here you can define the synchronization method like using the xref:vfs.adoc[Virtual Filesystem] or the location for synchronized data.
...
** menu:Reconnect[] +
May be necessary if there was an unrecoverable network problem and the account was disconnected.
...
To add accounts to your Desktop App, click on the menu:Add Account[] image:using/kw-windows-add-account-icon.png[Add Account Icon, width=18] button on the top bar and follow the xref:connection-wizard[Connection Wizard]. The added account will appear as a new item in the main window at the top.

==== Removing an Account

To remove an account, select the account you want to remove in the top bar and click the menu:Manage Account[Remove] to remove it. Note that the account will be removed but no synchronized data deleted. Synchronized data needs to be removed manually. A good rule of thumb is to open the sync location with File Explorer before removing the account, which makes it easier to remove the data.

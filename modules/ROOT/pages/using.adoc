...
The Desktop App can not only be fully closed with the Quit image:using/quit-symbol.png[Quit Synbol, width=18] symbol on the top right, but also be closed to the system tray to run continuously in the background.

* A btn:[right-click] on the {ini_name} systray icon opens a menu for quick access to multiple operations.
+
--
...
The Desktop App uses the following icons. See the table for more information about their meaning.
...
The Desktop App provides overlay icons, if not using the VFS feature, in addition to the normal file type icons, for your system file manager (Explorer on Windows, Finder on Mac and Nautilus on Linux) to indicate the sync status of your {ini_name} files.
